...
function generateImageFile(outFile, paths) {
   ...
   const pixel = featureFunctions.getPixel(paths);
   const complexity = pixel.filter((a)=> a != 0).length;
   draw.text(ctx, complexity, "gold")

   // const { hull} =geometry.minimumBoundingBox({
   //    points: paths.flat()
   // });
   
   // const roundness = geometry.roundness(hull);
   // const R = 255 - Math.floor((roundness **3) * 255);
   // const G = 255 -0;
   // const B = 255 - Math.floor((1- roundness**3) * 255);
